...
const PackageDetail = () => {
  ...
  const handleUpdatePackage = (updatedPackageData) => {
    setPackages((prev) =>
      prev.map((p) =>
        p.id === updatedPackageData.id ? updatedPackageData : p,
      ),
    );
    setIsEditFormOpen(false);
    toast({
      title: 'Package Updated',
      description: `${updatedPackageData.name} has been successfully updated.`,
    });
  };
  ...
  const handleDeletePackage = () => {
    ...
    toast({
      ...
      description: `${pkg?.name || 'Package'} has been deleted.`,
      variant: 'destructive',
    });
  ...
  return (
    <motion.div
      ...
      <div className="flex items-center justify-between">
        <Button
          variant="outline"
          onClick={() => navigate('/hosting/packages')}
          className="mb-4">
      ...
      <Card className="shadow-xl border-border/80">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start">
            <div>
              ...
              <CardDescription className="text-lg">
                {pkg.hostingType} for {pkg.targetAudience}
              </CardDescription>
            </div>
            <Badge
              variant="secondary"
              className="mt-2 sm:mt-0 text-2xl px-4 py-2 bg-sky-500/20 text-sky-400 dark:text-sky-300 border-sky-500/50">
        ...
        <CardContent className="p-6 grid md:grid-cols-2 gap-x-8 gap-y-6">
          <div className="space-y-4">
            <h3 className="text-xl font-semibold text-primary mb-2 border-b pb-2">
              Core Specifications
            </h3>
            <div className="flex items-center gap-3 text-card-foreground">
              ...
              />
              <span>
                <strong>Storage:</strong> {pkg.storage || 'N/A'}
              </span>
            </div>
            <div className="flex items-center gap-3 text-card-foreground">
              <Zap size={20} className="text-amber-500 dark:text-amber-400" />
              <span>
                <strong>Bandwidth:</strong> {pkg.bandwidth || 'N/A'}
              </span>
            </div>
            <div className="flex items-center gap-3 text-card-foreground">
              <Server
              ...
              />
              <span>
                <strong>Websites:</strong> {pkg.websites || 'N/A'}
              </span>
            </div>
            <div className="flex items-center gap-3 text-card-foreground">
              <MailIcon
              ...
              />
              <span>
                <strong>Email Accounts:</strong> {pkg.emailAccounts || 'N/A'}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 text-card-foreground">
              ...
              />
              <span>
                <strong>Hosting Type:</strong> {pkg.hostingType || 'N/A'}
              </span>
            </div>
            <div className="flex items-center gap-3 text-card-foreground">
              <Users2 size={20} className="text-lime-500 dark:text-lime-400" />
              <span>
                <strong>Target Audience:</strong> {pkg.targetAudience || 'N/A'}
              </span>
            </div>
          </div>
