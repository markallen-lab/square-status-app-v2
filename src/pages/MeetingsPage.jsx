...
const MeetingForm = ({ meetingData, onSubmit, onCancel, clients }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...
      participants,
      description,
      clientIds, // this stays an array of IDs
    });
  };
  ...
  return (
    <form
      ...
      <div>
        <Label>Link to Clients</Label>
        <ReactSelect
          isMulti
          options={clientOptions}
          value={clientOptions.filter((opt) => clientIds.includes(opt.value))}
          onChange={(selectedOptions) =>
            setClientIds(selectedOptions.map((opt) => opt.value))
          }
          className="react-select-container"
          classNamePrefix="react-select"
        />
        <p className="text-xs text-muted-foreground mt-1">
          You can link this meeting to multiple clients.
        </p>
      </div>
      <div>
      ...
      <DialogFooterPrimitive className="pt-4">
        ...
        <Button type="submit">
          {meetingData?.id ? 'Update Meeting' : 'Add Meeting'}
        </Button>
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-7 gap-1 mt-2">
          {Array.from({ length: firstDayOfMonth }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="border border-transparent rounded-md"></div>
          ))}
...
const MeetingsPage = () => {
  ...
  return (
    <motion.div
          ...
          <DialogContent className="sm:max-w-lg bg-card border-border">
            <DialogHeader>
              <DialogTitle className="text-primary">
                {editingMeeting?.id ? 'Edit Meeting' : 'Schedule New Meeting'}
              </DialogTitle>
            </DialogHeader>
            <MeetingForm
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="shadow-md bg-card border-border lg:col-span-1">
          <CardHeader>
            ...
            <CardDescription>
              Upcoming appointments for the selected day.
            </CardDescription>
          </CardHeader>
                ...
                <motion.div
                  ...
                  <div className="text-xs text-muted-foreground space-y-0.5 mt-1">
                    <p className="flex items-center">
                      <Clock size={12} className="mr-1.5" /> {meeting.time}{' '}
                    </p>
                    <p className="flex items-center">
                      <Users size={12} className="mr-1.5" /> Participants:{' '}
                      {meeting.participants || 'N/A'}
                    </p>
                    {meeting.clientIds && meeting.clientIds.length > 0 && (
                      <p className="flex items-center">
                        <Briefcase size={12} className="mr-1.5" /> Clients:{' '}
                        {getClientNamesForMeeting(meeting.clientIds)}
                      </p>
                    )}
                    {meeting.description && (
      ...
      <img
        alt="Modern abstract art representing calendar and scheduling"
        className="w-full max-w-2xl mx-auto h-auto rounded-lg shadow-lg mt-6"
        src="https://images.unsplash.com/photo-1541848980489-5719ac353a10"
      />
    </motion.div>
  );
};
